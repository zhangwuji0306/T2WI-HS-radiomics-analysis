...
- R6-5R first-failure coordinate reconciliation 已完成但未获下游接受：Path A 的历史 exact replay runner 无法恢复，已登记 `historical_diagnostic_runner_not_exactly_recoverable`；Path B 仅对 `repeat=1 / outer_fold=1 / R_high` 使用冻结 W07 split、training-only provider、P3B 和既有 eligibility 得到 full outer `314/79`、R_high `282/67`。第一轮 `Luna/xhigh` Reviewer 判定 `not accepted for downstream use`（项目原生含义：`HARD HOLD`），因为批准裁定的 I/II/III 分支均要求 current canonical 为 `288/68`，而当前结果为 `282/67`。R6-2 历史 `282/67` 未被重写；R6-5 数值等价性、R6-6/G3R、正式 W08、性能评估及 Cox 拟合均不得启动。B access 四项仍为 false，`model_freeze_lock.json` 不存在。
