...
- 已加入 13 个合成数据回归测试，覆盖标签解析、归一化隔离、强制覆盖、断点续跑、流水线印章、倾斜 FOV 和 SLIC物理尺度换算。
...
- 已完成修正后18例M1/M2比较、A=393校正M1、结构状态、local-global、20次bootstrap冒烟、技术因素效应量及严格A=137核验；校正后A=393硬技术失败0例，结构性单生境25例。正式1000次bootstrap及冻结门禁仍待长任务确认，结局盲态保持。
...
在结局盲态下完成正式1000次患者层面bootstrap并执行冻结前全部门禁；满足冻结条件后再纳入A集预设临床变量与DFS。B集在全A参数、特征和模型冻结前保持不可见。
