...
bounded synthetic technical probe 的三次同机重复中，集成总 probe 中位耗时相对 baseline 下降 0.93%；该结果低于 20% 阈值，因此复杂缓存/并行层不启用于正式执行。正式本地参数唯一保持 `representation_workers=2`、`outer_fold_workers=2`、每 worker 数值线程为 1。
...
- 当前导入后的 threadpool 观测：MKL=1；OpenMP=1（所有已加载 OpenMP pool 均为 1）。
...
|---|---:|---:|
| representation seconds | 0.152783 | 0.159278 |
| single-fold model seconds | 0.191642 | 0.182037 |
| total technical probe seconds | 0.345073 | 0.341871 |
