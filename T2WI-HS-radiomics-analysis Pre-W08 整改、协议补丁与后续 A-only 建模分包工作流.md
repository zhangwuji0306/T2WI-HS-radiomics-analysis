...
- 锁定 `t2_radiomics` 环境已恢复。R6-2 first-failure diagnostic replay 已完成并经第一轮独立 Reviewer 接受下游分类；R6-3 已完成第一轮独立复核，唯一推荐分类为 Class C，机器可读 validation=`PASS`，当前等待 R6-4 protocol-owner disposition。正式 W08、性能评估、W09 与 B validation 继续保持禁止。
